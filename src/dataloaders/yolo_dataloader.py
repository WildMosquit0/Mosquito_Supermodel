...
class YOLODataModule(pl.LightningDataModule):
    def __init__(self, config: dict):
        super().__init__()
        self.config = config
        self.batch = config['training'].get('batch', 16)
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch=self.batch, shuffle=True)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch=self.batch)
